  ...
  for (let d = 0; d <= maxDepth; d++) {
    ...
    for (const id of ids) {
      ...
      if (best >= 0) {
        ...
        // Tiny deterministic jitter so siblings at the same fruit-cluster don't overlap
        const s = (id.charCodeAt(0) || 1) * 29 + (id.charCodeAt(1) || 3) * 17
        const j = (seededRand(s) - 0.5) * 0.22
        positions.set(id, new THREE.Vector3(x + j, y + j * 0.4, z + j))
      }
    }
  }
